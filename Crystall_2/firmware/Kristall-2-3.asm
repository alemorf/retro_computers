...
vinxru_nocursor:ds 1	; Отключение отбражения курсора, если FF
...
reboot:		ld	sp, STACK_TOP
		...
		ld	(defCursorAddr), hl
		; ОРИГИНАЛЬНЫЙ КОД: ld	hl, 300h
		call	vinxru_init	; ПАТЧ
		ld	(tapeFormat), hl
...
monitor:	ld	sp, STACK_TOP
		...
		cp	52h ; 'R'
		jp	z, cmd_r
		; ОРИГИНАЛЬНЫЙ КОД: nop
		; ОРИГИНАЛЬНЫЙ КОД: nop
		; ОРИГИНАЛЬНЫЙ КОД: nop
		; ОРИГИНАЛЬНЫЙ КОД: nop
		; ОРИГИНАЛЬНЫЙ КОД: nop
		cp	'X'			; ПАТЧ
		jp	z, vinxru_cmd_x		; ПАТЧ
		nop
		nop
		nop
		nop
...
putCharA:	push	hl
		...
		ld	(hl), a
		ld	a, c
		; ОРИГИНАЛЬНЫЙ КОД: cp	1Fh
		; ОРИГИНАЛЬНЫЙ КОД: jp	z, clearScreen
		nop			; ПАТЧ
		nop			; ПАТЧ
		jp	vinxru_esc	; ПАТЧ
putChar_er:	cp	8
...
saveCursorAddr:	ld	(cursorAddr), hl
		ld	a, (hl)
		ld	(charAtCursor),	a
		; ОРИГИНАЛЬНЫЙ КОД: ld	(hl), 1Eh
		; ОРИГИНАЛЬНЫЙ КОД: ld	a, c
		call vinxru_cursor	; ПАТЧ
popa_ret:	pop	af
		pop	bc
		pop	de
		pop	hl
...
vinxru_init:	xor	a
		ld	(vinxru_nocursor), a
		ld      (vinxru_escFlag), a
		ld	hl, 300h	; удаленный оригинальный код
		ret
...
vinxru_cursor:	ld	a, (vinxru_nocursor)
		and	a
		ret	nz
		ld	(hl), 1Eh
		ret
...
vinxru_cmd_x:	ld hl, vinxru_nocursor
		ld a, (hl)
		xor 1
		ld (hl), a
		ret
