...
rom:		jp	reboot
		...
		jp	_loadProgramm		; ОРИГИНАЛ: jp	initKeyb
...
putCharA:	ld	c, a			; ОРИГИНАЛ: ld a, c
putCharC:	push	hl			
		push	de
		push	bc
		push	af
...
;_loadProgramm:	ld	a, 0Dh
;		ld	(cmdLine+1), a
;		call	cmd_r
;		; В bc возвращается контрольная сумма. Только 
;		; рассчитанная, а не с ленты.
;		ld	hl, (cmd_param3)
;		ex	hl, de
;		ld	hl, (cmd_param1)
;		ret
...
; Параметр BC игнорируется. 

_saveProgramm:	ld bc, 0		; Не изменять скорость
		jp cmd_o
...
;============================================================================
		...
		savebin "Kristall-2.dat",rom,10000h-rom      ё