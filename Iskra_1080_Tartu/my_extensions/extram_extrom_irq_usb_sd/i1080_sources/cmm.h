...
#define PushPop(X...) for (bool _cmmExit = ({ push(X), true; }); _cmmExit; _cmmExit = false, pop(X))
...
// TODO: const
static RegisterA& CarryAdd(RegisterA& x, Register8& x) {
    assert(false);  // TODO
    return x;
}

// TODO: const
static RegisterA& CarrySub(RegisterA& x, Register8& x) {
    assert(false);  // TODO
    return x;
}
