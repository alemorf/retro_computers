...
void LoadFile(const char file_name[], size_t min_file_size, size_t max_file_size, std::vector<uint8_t> *out) {
    // Check parameters
    if (file_name == nullptr) {
        throw std::runtime_error(std::string(__func__) + std::string(": Incorrect parameter file_name = nullptr"));
    }
    ...
    FsTools::LoadFile(file_name, [file_name, out, min_file_size, max_file_size](size_t size) {
