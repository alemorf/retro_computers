...
jmpparam1 equ 63348
 .org 0xF800
; 66 extern uint8_t rst30Opcode __address(0x30);
...
; 437         if ((a = inputBuffer + sizeof(inputBuffer) - 1) == l)
	ld a, 0FFh & (0FFFFh & ((0FFFFh & ((inputbuffer) + (32))) - (1)))
	cp l
...
; 1422 }
; 1423 
; 1424 const uint8_t ZERO_LINE = (SCREEN_BEGIN >> 6) & 0xFF;
; 1425 
...
; 1479             l--;
	dec l
l_143:
; 1480         } while ((a = l) != ((SCREEN_BEGIN + SCREEN_WIDTH * SCREEN_HEIGHT - 1 - SCREEN_WIDTH) & 0xFF));
	ld a, l
	cp 255
	jp nz, l_142
	pop hl
...
; 1722     h = (uintptr_t)specialKeyTable >> 8;
	ld h, 0FFh & (0FFFFh & ((specialkeytable) >> (8)))
; 1723     l = (a += (uintptr_t)specialKeyTable - 48);
	add 0FFh & (0FFFFh & ((specialkeytable) - (48)))
	ld l, a
; 1724     a = *hl;
	ld a, (hl)
; 1725     ScanKeyExit(a);
...
aregisters:
	...
	db 25
	ds 1
; 1743 uint8_t aBackspace[] = "\x08 \x08";
abackspace:
	db 8
	db 32
	db 8
	ds 1
; 1744 uint8_t aHello[] = "\x1FМИКРО 80\x1BБ";
ahello:
	db 31
	db 109
	db 105
	...
	db 98
	ds 1
; 1751 uint8_t aZag[3] = "Заг";
azag:
	db 122
	db 225
	db 231
; 1752 uint8_t aStr[3] = "Стр";
astr:
	db 115
	db 244
	db 242
; 1753 uint8_t aLat[3] = "Лат";
alat:
	db 108
	db 225
	db 244
; 1754 uint8_t aRus[3] = "Рус";
arus:
	db 114
	db 245
	db 243
; 1756 void PrintKeyStatus() {
printkeystatus:
