...
START_FROM_RAM	= 1             ; Запуск с внешнего носителя. Внешний загрузчик заполняет
				; первые 32 Кб памяти образом диска. Система запускается из ОЗУ, 
				; так же как из ПЗУ. В качестве накопителя A: будет выступать 
				; стандартная оперативной память. При запуске
				; будет вызываться CONFIG.BAT, который будет устаналивать
				; драйвер внешнего носителя в качестве накопителя A:
...
aANc_com:	.text "A:NC.COM"
		.db 0Dh
#if START_FROM_RAM
aBAutoex_bat:	.db "A:AUTOEX.BAT"
#else
aBAutoex_bat:	.db "B:AUTOEX.BAT"
#endif
		.db 0Dh
aBat:		.text "BAT"
aCom:		.text "COM"
